...
public:
	...
	//void drawCG(CGContextRef ctx);   //delete by @solysky20200814
    ...
    void drawMetalColor(AAPLVertex *vertex, float size, float r, float g, float b, float a);
...
inline void
Grain::drawMetalColor(AAPLVertex *vertex, float size, float r, float g, float b, float a)
{
    vertex->position.x = pos_.x;
    vertex->position.y = pos_.y;
    vertex->size = size;
    vertex->color.r = r;
    vertex->color.g = g;
    vertex->color.b = b;
...
Grain::drawVertexArrayColor(ofxMSAShape3D *vertexArray, Color *colors, int nColors)
{
	...
	vertexArray->setColor(components[0], components[1], components[2], 1.);
