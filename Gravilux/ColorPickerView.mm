...
static float colorLuminance( CGColorRef colorToAnalyze )
{
...
static float colorBrightness( CGColorRef colorToAnalyze )
{
	const CGFloat* components = CGColorGetComponents(colorToAnalyze);
...
//
//
static float colorRed( CGColorRef colorToAnalyze )
{
	const CGFloat* components = CGColorGetComponents(colorToAnalyze);
	return components[0];        
}
...
//
//
static float colorGreen( CGColorRef colorToAnalyze )
{
	const CGFloat* components = CGColorGetComponents(colorToAnalyze);
	return components[1];        
}
...
//
//
static float colorBlue( CGColorRef colorToAnalyze )
{
	const CGFloat* components = CGColorGetComponents(colorToAnalyze);
	return components[2];        
}
...
//
//
static float colorAlpha( CGColorRef colorToAnalyze )
{
	const CGFloat* components = CGColorGetComponents(colorToAnalyze);
	return components[3];        
}
...
//
//
static float colorHue( CGColorRef colorToAnalyze )
{
	const CGFloat* components = CGColorGetComponents(colorToAnalyze);
...
//
//
static float colorSat( CGColorRef colorToAnalyze )
{
	const CGFloat* components = CGColorGetComponents(colorToAnalyze);
	float r = components[0];        
	float g = components[1];        
...
- (UIColor *)colorOfImage:(CGImageRef)img withData:(NSData*)imageData atPoint:(CGPoint)p
{
	...
	unsigned long bytesPerRow = CGImageGetBytesPerRow(img);
	unsigned long  bytesPerPixel = CGImageGetBitsPerPixel(img) / 8;
	unsigned long  byteIndex = (bytesPerRow * roundf(p.y)) + bytesPerPixel * roundf(p.x);
