...
bool Parameters::loadPreset(int num, bool colorsOnly)
{
	...
	if (!colorsOnly) {
		...
		long r = [prefs integerForKey:KEYSTRING(KEY_COLS,num)];
		long c = [prefs integerForKey:KEYSTRING(KEY_ROWS,num)]; 
		if (r != 0 && c != 0) setSize((int) r, (int) c);
...
//    NSLog(@"setColors: r,g,b = { {%.2f, %.2f, %.2f}, {%.2f, %.2f, %.2f}, {%.2f, %.2f, %.2f} }",
//          colors_[0].r, colors_[0].g, colors[0].b,
//          colors_[1].r, colors_[1].g, colors[1].b,
//          colors_[2].r, colors_[2].g, colors[2].b);
...
void Parameters::setDefaultColors()
{
	...
	colors_[2].g = 0.1;
	colors_[2].b = 0.4;
	[[NSNotificationCenter defaultCenter] postNotificationName:@"updateSettingsUI" object:[NSNotificationCenter defaultCenter]];
}
