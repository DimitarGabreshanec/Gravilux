...
public:
    ...
    void drawMetal(AAPLVertex *metalVertices);
...
//	void touchesEnded(NSSet * touches);
    ...
    unsigned int nGrains() { return nGrains_; }
    CGSize screenSize() { return ((UIScreen*)[[UIScreen screens] objectAtIndex:0]).bounds.size; }
...
//	InterfaceViewController		*interfaceViewController_;
	...
	unsigned int			nGrains_;
	unsigned int			nAllocGrains_;
...
//	bool isShaking(float threshold);
	...
	void drawGrains();
    UIColor* getColorFromImage(UIImage* image, int xx, int yy);
};
