...
Gravilux::renderType(NSString * text, UIInterfaceOrientation orientation)
{
    ...
    CGSize wordSizeAtCurrentFont;
    for (NSString * word in words) {
#ifdef __IPHONE_7_0
        {
            sizeAttributes = @{NSFontAttributeName:font, NSParagraphStyleAttributeName: style};
            CGRect frame = [word boundingRectWithSize:fontSizeConstraint options:NSStringDrawingUsesLineFragmentOrigin attributes:sizeAttributes context:nil];
            wordSizeAtCurrentFont = frame.size;
        }
#else
        {
    ...
    CGSize textSizeAtCurrentFont;
#ifdef __IPHONE_7_0
    sizeAttributes = @{NSFontAttributeName:font, NSParagraphStyleAttributeName: style};
    CGRect frame = [text boundingRectWithSize:fontSizeConstraint options:NSStringDrawingUsesLineFragmentOrigin attributes:sizeAttributes context:nil];
    textSizeAtCurrentFont = frame.size;
#else
...
#ifdef __IPHONE_7_0
    sizeAttributes = @{NSFontAttributeName:font, NSParagraphStyleAttributeName: style};
    frame = [text boundingRectWithSize:fontSizeConstraint options:NSStringDrawingUsesLineFragmentOrigin attributes:sizeAttributes context:nil];
    textSize = frame.size;
#else
...
#ifdef __IPHONE_7_0
    NSMutableParagraphStyle *styles = [[NSMutableParagraphStyle defaultParagraphStyle] mutableCopy];
    [styles setLineBreakMode: NSLineBreakByWordWrapping];
    [styles setAlignment:NSTextAlignmentCenter];
    NSAttributedString *richText = [[NSAttributedString alloc] initWithString:text attributes:@{NSFontAttributeName: font, NSParagraphStyleAttributeName: styles, NSForegroundColorAttributeName: [UIColor whiteColor] }];
    CGRect textRect = [richText boundingRectWithSize:fontSizeConstraint options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading context:nil];
    textRect = CGRectMake(drawingArea.origin.x, drawingArea.origin.y, drawingArea.size.width, textRect.size.height);
    ...
    //    CGImageRef imageRef = uiImage.CGImage;    // for looking at image in debugger
...
UIColor* 
Gravilux::getColorFromImage(UIImage* image, int xx, int yy)
{
...
Gravilux::drawInContext(CGContext *ctx)
{
	...
	for (v = 0; v < nGrains_; v++) {
		if (params_->heatColor()) {
			//grains_[v]->drawColorCG(gradientColors, nColors, ); //ysky//
		} else {
			//grains_[v]->drawCG(ctx);
            //grains_[v]->drawColorCG(ctx, gradientColors, 3); //ysky
		}
	}
}	
...
//			if (pvmax > maxVelocity_) maxVelocity_ = pvmax; // sss removed 12/21/2020 b/c it seemed to add visual glitches
...
void
Gravilux::drawMetal(AAPLVertex *metalVertices)
{
    int g, i1, i2;
    float v, pvmax, vNorm, interp, oneMinusInterp;
    Color gradientColors[3];
    ...
    if (params_->heatColor()
        ...
        for (g = 0; g < nGrains_; g++) {
            ...
            grains_[g]->drawMetalColor(&metalVertices[g],
                                       params_->deviceStarSize(),
                                       gradientColors[i1].r * oneMinusInterp + gradientColors[i2].r * interp,
                                       gradientColors[i1].g * oneMinusInterp + gradientColors[i2].g * interp,
                                       gradientColors[i1].b * oneMinusInterp + gradientColors[i2].b * interp,
                                       1.);
        }
